...
import { Pressable, StyleSheet, Switch, Text, TextInput, View, Platform, Alert, ActivityIndicator } from 'react-native';
...
export function TimeTemplate({ automationId }: { automationId?: number }) {
  const { activeHome } = useHome();
  const [name, setName] = useState('Hẹn giờ thiết bị');
  const [enabled, setEnabled] = useState(true);
  const [isSaving, setIsSaving] = useState(false);
  const [isLoading, setIsLoading] = useState(!!automationId);
  
  // Date and Time picker state
  const [date, setDate] = useState(new Date(new Date().setHours(23, 30, 0)));
  ...
  useEffect(() => {
    if (automationId && activeHome) {
      Promise.all([
        ...
        if (data.trigger?.type === 'time' && data.trigger.value) {
          setTime(String(data.trigger.value));
          const [h, m] = String(data.trigger.value).split(':');
          if (h && m) {
            const newDate = new Date();
            newDate.setHours(Number(h), Number(m), 0);
            setDate(newDate);
          }
        }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.sectionHeader}>
        <Text style={styles.sectionLabel}>THỜI GIAN (KHI)</Text>
      </View>

      <Pressable onPress={() => setShowPicker(true)} style={({ pressed }) => [styles.timePickerCard, pressed && { opacity: 0.8 }]}>
        <View style={styles.timeIconWrap}>
          <Ionicons color={colors.primary} name="time-outline" size={28} />
        </View>
        <Text style={styles.timeText}>{time}</Text>
        <View style={styles.timeBadge}>
          <Text style={styles.timeBadgeText}>Hằng ngày</Text>
        </View>
      </Pressable>

      {/* Renders the DateTimePicker when showPicker is true */}
      {showPicker && (
...
const styles = StyleSheet.create({
  ...
  timeText: { fontSize: 42, fontWeight: '800', color: colors.primaryPressed, letterSpacing: 2 },
  timeBadge: {
    marginTop: 12,
  ...
  timeBadgeText: { color: colors.surface, fontSize: 13, fontWeight: '700' },
