...
Hệ thống IoT Nhà thông minh & Nông nghiệp thông minh tích hợp vi điều khiển **Yolo:Bit (ESP32-S3)**, cảm biến môi trường (DHT20, HC-SR04), cơ cấu chấp hành (Đèn LED RGB 4 bóng, Máy bơm nước), màn hình LCD1602, Remote hồng ngoại (IR) và kết nối Cloud hai chiều với **Adafruit IO qua giao thức MQTT**.
...
│  • P1: IR Remote Receiver       • P2: Water Pump / Relay  │
│  • I2C (P19/P20): DHT20 (Temp/Humi) + LCD1602 Screen   │
└────────────────────────────────────────────────────────┘
```
...
| Module / Cảm Biến | Cổng Cắm (Port) | Chân GPIO Yolo:Bit | Mô Tả Chức Năng |
| :--- | :---: | :---: | :--- |
...
| **Máy Bơm Nước (Relay/Driver)** | **`P2`** | `GPIO 5` | Cắm module đóng ngắt vào **`P2`**, cắm máy bơm vào **USB Output 1**. |
...
| **Màn Hình LCD1602 (I2C)** | **`I2C (P19/P20)`** | SCL (`P19`), SDA (`P20`) | Hiển thị thông số thời gian thực và trạng thái thiết bị. Địa chỉ: `0x27`. |

> 💡 *Mẹo: Màn hình LCD và Cảm biến DHT20 cắm chung vào cụm cổng I2C (thông qua Hub chia I2C).*
...
| :--- | :---: | :---: | :--- |
| **`bbc-temp`** | Cloud ◄── Board | Số thực (VD: `29.2`) | Dữ liệu nhiệt độ môi trường đo từ DHT20. |
| **`bbc-humi`** | Cloud ◄── Board | Số thực (VD: `71.5`) | Dữ liệu độ ẩm môi trường đo từ DHT20. |
| **`bbc-led1`** | Cloud ──► Board | `1` / `0` | Bật / Tắt **LED 1**. |
| **`bbc-led2`** | Cloud ──► Board | `1` / `0` | Bật / Tắt **LED 2**. |
| **`bbc-led3`** | Cloud ──► Board | `1` / `0` | Bật / Tắt **LED 3**. |
| **`bbc-led4`** | Cloud ──► Board | `1` / `0` | `1` = Bật giữ sáng LED 4 (Override).<br>`0` = Tắt & trả về chế độ tự động cảm biến khoảng cách. |
...
1. **Điều khiển Đèn LED 1, 2, 3:**
   * Bật/tắt thủ công độc lập từ Adafruit IO hoặc bấm phím `1`, `2`, `3` trên Remote.
...
3. **Điều khiển Máy Bơm Nước:**
   * Bật/tắt tức thì qua feed `bbc-pump` (`1` = Chạy, `0` = Dừng).
4. **Màn hình LCD1602:**
...
### 1. Cài đặt thư viện Python (Máy tính)
Mở terminal tại thư mục dự án và chạy:
```powershell
pip install -r requirements.txt
```

### 2. Nạp Firmware vào Board Yolo:Bit (Chỉ cần chạy 1 lần khi cập nhật code)
