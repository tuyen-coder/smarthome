...
except Exception as e:  # noqa: BLE001 - Adafruit client errors are not normalized
...
try:
    while True:
        ...
        try:
            ...
            print(
                f"[{time.strftime('%H:%M:%S')}] Simulated TEMP = {temp_val}°C "
                "-> Sent successfully to 'bbc-temp'"
            )
        except Exception as e:  # noqa: BLE001 - continue the simulator loop
        ...
        try:
            ...
            print(
                f"[{time.strftime('%H:%M:%S')}] Simulated HUMI = {humi_val}% "
                "-> Sent successfully to 'bbc-humi'"
            )
        except Exception as e:  # noqa: BLE001 - continue the simulator loop
...
except Exception as e:  # noqa: BLE001 - report unexpected simulator failures
